...
function renderTreeNodes(nodes: TreeNodeLike[], depth: number): string {
  ...
  for (const node of nodes) {
    out.push(
      `<li class="tree-node" data-depth="${depth}" data-object-id="${escapeHtml(node.id)}">` +
...
function renderObjectTree(view: DesktopViewSnapshot): string {
  return `<ul class="tree-list">${renderTreeNodes(view.objectTree as TreeNodeLike[], 0)}</ul>`;
}

function renderCanvas(view: DesktopViewSnapshot): string {
...
function renderProperties(view: DesktopViewSnapshot): string {
  if (!view.properties) return '<p class="muted">No object selected.</p>';
  const core = [
...
function renderImportReport(view: DesktopViewSnapshot): string {
  if (!view.importReport) return '<p class="muted">No import report available.</p>';
...
export function renderDesktopShellDocument(input: DesktopShellRenderInput): string {
  const statePayload = {
    window: input.window,
