...
const buildProvenance_1 = require("./provenance/buildProvenance");
const stabilityProfile_1 = require("./stabilityProfile");
const NON_RENDERABLE_DEFINITION_TAGS = new Set([
    'defs',
    'symbol',
    'clippath',
    'mask',
...
function defaultBase(nodeId, explicitId, objectType, bbox, provenance, capabilities) {
    return {
        ...
        capabilities,
        provenance,
        stability: (0, stabilityProfile_1.makeStabilityProfile)(objectType === 'image_node' ? 'atomic' : objectType === 'group_node' || objectType === 'html_block' ? 'group_only' : 'semantic'),
        manualEdits: [],
    };
}
const HTML_TAGS = new Set(['div', 'span', 'p', 'img', 'figure', 'figcaption', 'section']);
function resolveHtmlTagName(node) {
    const explicit = String(node.attributes['data-fe-tag'] ?? '').toLowerCase();
    if (HTML_TAGS.has(explicit))
        return explicit;
    const native = node.tagName.toLowerCase();
    if (HTML_TAGS.has(native))
        return native;
    return 'div';
}
function resolveNodeTextContent(node) {
...
function unionBBoxes(boxes) {
    const valid = boxes.filter((bbox) => Boolean(bbox));
    if (valid.length === 0)
        return null;
    let minX = valid[0].x;
    let minY = valid[0].y;
...
function collectDescendantNodeIds(normalized, rootNodeId) {
    const out = [];
...
function buildAttributeIdIndex(normalized) {
    const index = new Map();
...
function isSerializableShapeTag(tagName) {
    return ['path', 'line', 'rect', 'circle', 'polyline', 'polygon', 'ellipse'].includes(tagName);
}
function resolveShapeKind(node, useReference) {
...
function buildUseReference(node, normalized, parentById, attributeIdIndex) {
    if (String(node.localTagName ?? node.tagName ?? '').toLowerCase() !== 'use')
        return null;
    ...
    if (!isSerializableShapeTag(definitionTag))
        return null;
    if (!isInNonRenderableDefinitionSubtree(definitionNode.nodeId, normalized, parentById))
        return null;
    return {
        href: `#${definitionId}`,
        definitionId,
...
function isMarkerLikeHrefToken(raw) {
    if (!raw)
        return false;
    return /^m[0-9a-f]{4,}$/i.test(raw);
}
function isFontGlyphHref(href) {
    const raw = normalizeHrefToken(href);
    if (!raw || isMarkerLikeHrefToken(raw))
        return false;
    return /-([0-9a-fA-F]{1,6})$/.test(raw) || /^[A-Za-z][0-9A-Fa-f]{2,6}$/.test(raw);
}
function decodeGlyphCharFromHref(href) {
    const raw = normalizeHrefToken(href);
    if (!raw || isMarkerLikeHrefToken(raw))
        return null;
...
function normalizeGlyphFontFamilyToken(token) {
...
function decodeGlyphContent(node, normalized, attributeIdIndex) {
    ...
    if (glyphUseNodes.length === 0)
        return null;
    if (glyphUseNodes.length / useNodes.length < 0.6)
        return null;
    ...
    if (content.trim().length === 0 && !content.includes(' '))
        return null;
    const descendantTransformCarrier = descendants.find((child) => child.localTagName.toLowerCase() === 'g' && child.transform.raw !== null) ??
    ...
    const scaleDrivenFontSize = transformCarrier.transform.raw && (Math.abs(scaleX - 1) > 0.001 || Math.abs(scaleY - 1) > 0.001)
    ...
    const fallbackGlyphSizes = glyphUseNodes
        ...
        .filter((childBBox) => Boolean(childBBox))
    ...
    const widthFromAdvance = rawXOffsets.length > 0
    ...
    for (const useNode of glyphUseNodes) {
        ...
        if (!definitionId || definitionMap.has(definitionId))
            continue;
        const definitionNode = attributeIdIndex.get(definitionId);
        if (!definitionNode)
            continue;
...
function makeGlyphTextProxy(node, normalized, source, attributeIdIndex) {
    ...
    if (!looksLikeExportedProxy && !/^text_\d+$/i.test(idAttr))
        return null;
    ...
    const proxy = {
        ...defaultBase(node.nodeId, node.attributes['data-fe-object-id'], 'text_node', bbox, (0, buildProvenance_1.buildProvenance)({
...
function makeTextObject(node, source) {
    ...
    const capabilities = isProxy
        ? ['select', 'multi_select', 'drag', 'delete', 'group_only']
        : ['select', 'multi_select', 'drag', 'delete', 'text_edit'];
    return {
        ...defaultBase(node.nodeId, node.attributes['data-fe-object-id'], 'text_node', textBBox, (0, buildProvenance_1.buildProvenance)({
            sourceId: source.sourceId,
            node,
            originFileKind: source.kind === 'html' ? 'html' : 'svg',
...
function makeImageObject(node, source) {
    const capabilities = ['select', 'multi_select', 'drag', 'delete', 'crop_only', 'replace_image', 'resize'];
    return {
        ...defaultBase(node.nodeId, node.attributes['data-fe-object-id'], 'image_node', node.bbox ?? { x: 0, y: 0, w: 0, h: 0 }, (0, buildProvenance_1.buildProvenance)({
            sourceId: source.sourceId,
            node,
            originFileKind: source.kind === 'html' ? 'html' : 'svg',
            liftedBy: 'generic_importer',
...
function makeShapeObject(node, source, normalized, parentById, attributeIdIndex) {
    const capabilities = ['select', 'multi_select', 'drag', 'delete', 'style_edit'];
    ...
    return {
        ...defaultBase(node.nodeId, node.attributes['data-fe-object-id'], 'shape_node', node.bbox ?? { x: 0, y: 0, w: 0, h: 0 }, (0, buildProvenance_1.buildProvenance)({
            sourceId: source.sourceId,
            node,
            originFileKind: source.kind === 'html' ? 'html' : 'svg',
            liftedBy: 'generic_importer',
...
function makeGroupOrHtmlObject(node, source, normalized) {
    ...
    const capabilities = objectType === 'html_block'
    ...
    const base = defaultBase(node.nodeId, node.attributes['data-fe-object-id'], objectType, node.bbox ?? { x: 0, y: 0, w: 0, h: 0 }, (0, buildProvenance_1.buildProvenance)({
        sourceId: source.sourceId,
        node,
        originFileKind: source.kind === 'html' ? 'html' : 'svg',
        liftedBy: 'generic_importer',
...
function buildParentIndex(normalized) {
    const parentById = {};
    for (const node of Object.values(normalized.nodes)) {
        if (parentById[node.nodeId] === undefined)
            parentById[node.nodeId] = null;
...
function isInNonRenderableDefinitionSubtree(nodeId, normalized, parentById) {
    let cursor = nodeId;
    while (cursor) {
        ...
        if (!node)
            break;
        const localTag = String(node.localTagName ?? node.tagName ?? '').toLowerCase();
        if (NON_RENDERABLE_DEFINITION_TAGS.has(localTag))
            return true;
        cursor = parentById[cursor] ?? null;
    }
    return false;
}
function mapAdapterToLiftedBy(adapterName) {
    switch (adapterName) {
        case 'manual_promote':
...
function semanticObjectExplicitId(node, expectedRole) {
    const role = String(node.attributes['data-fe-role'] ?? '').toLowerCase();
    if (expectedRole === 'panel' && role !== 'panel')
        return undefined;
    if (expectedRole === 'legend' && role !== 'legend')
        return undefined;
    return node.attributes['data-fe-object-id'] ?? undefined;
}
function resolvedObjectIdForNode(node) {
    return node.attributes['data-fe-object-id'] ?? `obj_${node.nodeId}`;
}
function reserveSemanticId(preferredId, role, nodeId, reservedIds) {
...
function buildSemanticChildRefs(wrappedId, explicitId) {
    if (!wrappedId)
        return [];
    if (wrappedId === explicitId)
        return [];
    return [wrappedId];
}
function isSemanticMarkerNode(node) {
    const role = String(node.attributes['data-fe-role'] ?? '').toLowerCase();
    return role === 'panel' || role === 'legend' || role === 'annotation' || role === 'annotation_block' || role === 'figure_title' || role === 'panel_label';
}
function buildSemanticObjects(normalized, hints, source, reservedIds) {
    const panels = [];
    const legends = [];
    ...
    for (const hint of hints.hints) {
        ...
        if (hint.kind === 'panel_candidate') {
            ...
            panels.push({
                ...defaultBase(`panel_${node.nodeId}`, explicitId, 'panel', node.bbox ?? { x: 0, y: 0, w: 0, h: 0 }, (0, buildProvenance_1.buildProvenance)({
                    sourceId: source.sourceId,
                    node,
                    originFileKind: source.kind === 'html' ? 'html' : 'svg',
                    ...
                    degradationReason: 'none',
                }), ['select', 'multi_select', 'drag', 'resize', 'delete', 'reparent']),
                label: null,
                titleObjectId: null,
        ...
        if (hint.kind === 'legend_candidate') {
            ...
            const wrappedId = resolvedObjectIdForNode(node);
            legends.push({
                ...defaultBase(`legend_${node.nodeId}`, explicitId, 'legend', node.bbox ?? { x: 0, y: 0, w: 0, h: 0 }, (0, buildProvenance_1.buildProvenance)({
                    sourceId: source.sourceId,
                    node,
                    originFileKind: source.kind === 'html' ? 'html' : 'svg',
                    liftedBy: mapAdapterToLiftedBy(hint.evidence[0]?.adapter),
                    liftConfidence: hint.confidence,
                    degradationReason: 'none',
                }), ['select', 'multi_select', 'drag', 'delete', 'reparent']),
...
function buildProject(normalized, hints, source) {
    const objects = {};
    ...
    for (const node of Object.values(normalized.nodes)) {
        ...
        if (skippedNodeIds.has(node.nodeId))
            continue;
    ...
    for (const p of semantic.panels)
        objects[p.id] = p;
    for (const l of semantic.legends)
        objects[l.id] = l;
    const figure = {
        figureId: 'fig_001',
