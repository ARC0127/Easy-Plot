import type { AnyObject, Project } from '../project';
import { validateCapabilityConflicts } from './capabilityValidator';
...
export function validateInvariants(project: Project): ValidationReport {
  const issues: ValidationIssue[] = [];
  const objects = Object.values(project.project.objects) as AnyObject[];
  ...
  for (const obj of objects) {
    ...
    if (obj.objectType === 'legend' || obj.objectType === 'figure_title' || obj.objectType === 'panel_label') {
      const anchoredObj = obj as AnyObject & { anchor?: unknown; offset?: unknown };
  ...
  for (const issue of validateCapabilityConflicts(project)) {
