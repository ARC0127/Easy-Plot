...
const manualEdits_1 = require("./manualEdits");
...
function approximateTextBBox(obj, content) {
...
function upgradeProxyTextForEditing(obj, content) {
    const before = {
    ...
    delete obj.glyphVector;
    (0, manualEdits_1.appendManualEdit)(obj, 'manual_role_override', before, {
        textKind: obj.textKind,
        content: obj.content,
        capabilities: [...obj.capabilities],
        bbox: { ...obj.bbox },
...
function childRefsOf(obj) {
    ...
    if (obj.objectType === 'panel') {
        if (obj.contentRootId)
            refs.add(obj.contentRootId);
        if (obj.titleObjectId)
            refs.add(obj.titleObjectId);
    }
    if (obj.objectType === 'legend') {
        for (const itemObjectId of obj.itemObjectIds)
            refs.add(itemObjectId);
    }
    return [...refs];
}
...
function updateChildRefs(project, removedId) {
    for (const obj of Object.values(project.project.objects)) {
        if ('childObjectIds' in obj && Array.isArray(obj.childObjectIds)) {
            obj.childObjectIds = obj.childObjectIds.filter((id) => id !== removedId);
        }
        if (obj.objectType === 'legend') {
...
function buildSemanticFromSelection(project, objectIds, role, reason) {
    if (objectIds.length === 0) {
        throw new index_1.FigureEditorError('ERR_INVALID_MANUAL_PROMOTION', 'Cannot promote empty selection.');
    }
    ...
    (0, manualEdits_1.appendManualEdit)(created, 'manual_promote', { objectIds, role: 'none' }, { objectIds, role }, reason);
    project.project.objects[id] = created;
    return id;
}
function applyOperation(project, operation) {
    const next = (0, undoRedo_1.pushSnapshot)(project);
    switch (operation.type) {
        ...
        case 'EDIT_TEXT_CONTENT': {
            ...
            if (!obj.capabilities.includes('text_edit')) {
                throw new index_1.FigureEditorError('ERR_TEXT_EDIT_UNSUPPORTED', `Object ${operation.payload.objectId} does not support text editing.`);
            }
            if (obj.objectType === 'text_node') {
                const before = { content: obj.content };
                obj.content = operation.payload.content;
                obj.bbox = approximateTextBBox(obj, obj.content);
                (0, manualEdits_1.appendManualEdit)(obj, 'manual_role_override', before, { content: obj.content }, 'edit_text');
            }
            else if (obj.objectType === 'html_block') {
                const before = { textContent: obj.textContent };
                obj.textContent = operation.payload.content;
                (0, manualEdits_1.appendManualEdit)(obj, 'manual_role_override', before, { textContent: obj.textContent }, 'edit_text');
            }
            else {
                throw new index_1.FigureEditorError('ERR_TEXT_EDIT_UNSUPPORTED', `Object ${operation.payload.objectId} does not support text editing.`);
            }
        ...
        case 'SET_ANCHOR': {
            const obj = getObject(next, operation.payload.objectId);
            if (!('anchor' in obj)) {
                throw new index_1.FigureEditorError('ERR_CAPABILITY_CONFLICT', `Object ${operation.payload.objectId} does not expose anchor.`);
            }
            const before = { anchor: obj.anchor, offset: obj.offset };
            obj.anchor = operation.payload.anchor;
            obj.offset = operation.payload.offset;
            (0, manualEdits_1.appendManualEdit)(obj, 'manual_anchor_change', before, { anchor: obj.anchor, offset: obj.offset }, 'set_anchor');
            break;
        }
        ...
        case 'UNGROUP_OBJECT': {
            ...
            if (obj.objectType !== 'group_node') {
                throw new index_1.FigureEditorError('ERR_INVALID_MANUAL_PROMOTION', `Object ${obj.id} is not a group_node.`);
            }
            delete next.project.objects[obj.id];
            next.project.figure.floatingObjects = removeId(next.project.figure.floatingObjects, obj.id);
            break;
    ...
    (0, undoRedo_1.appendOperationLog)(next, operation);
