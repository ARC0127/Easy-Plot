...
import { Panel, Project } from '../../../ir-schema/dist/index';
import { ConstraintMutationResult } from '../types';
import { cloneProject, getFigureBounds, getObject, pushConstraint } from '../helpers';

export function relayoutPanels(project: Project, mode: 'horizontal' | 'vertical' = 'horizontal', gap = 12): ConstraintMutationResult {
  const next = cloneProject(project);
  const panels = next.project.figure.panels
    .map(id => getObject(next, id))
    .filter((o): o is Panel => o.objectType === 'panel');
  if (panels.length < 2) return { project: next, appliedConstraint: null };
  const bounds = getFigureBounds(next);
  ...
  pushConstraint(next, mode === 'horizontal' ? 'equal_spacing_horizontal' : 'equal_spacing_vertical', panels.map(p => p.id), null, { gap });
