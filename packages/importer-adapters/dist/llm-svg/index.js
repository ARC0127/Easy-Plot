...
exports.detectLlmSvgHints = detectLlmSvgHints;
function makeHint(kind, nodeId, confidence, reason) {
    return {
        kind,
        nodeIds: [nodeId],
        confidence,
...
function detectLlmSvgHints(normalized) {
    const hints = [];
    ...
    for (const candidate of panelCandidates) {
        ...
        hints.push(makeHint('panel_candidate', candidate.nodeId, role === 'panel' ? 'high' : 'medium', role === 'panel' ? 'Matched exported data-fe-role=panel' : `Matched panel-like id: ${id}`));
    }
    ...
    for (const candidate of legendCandidates) {
        const role = String(candidate.attributes['data-fe-role'] ?? '').toLowerCase();
        const id = String(candidate.attributes.id ?? '');
        hints.push(makeHint('legend_candidate', candidate.nodeId, role === 'legend' || id.toLowerCase().includes('legend') ? 'high' : 'medium', role === 'legend'
