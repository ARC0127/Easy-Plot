...
exports.computeInteractionRetention = computeInteractionRetention;
function highValueCapabilities(obj) {
...
function findBestMatch(beforeObj, after) {
    const objects = Object.values(after.project.objects);
    const exact = objects.find((candidate) => candidate.id === beforeObj.id) ??
...
function computeInteractionRetention(before, afterReimport) {
    const failures = [];
    let originalCount = 0;
    let retainedCount = 0;
    for (const obj of Object.values(before.project.objects)) {
        ...
        if (ok)
            ...
            failures.push({ objectId: obj.id, expected, actual });
    }
