...
exports.computeSemanticBBoxDiff = computeSemanticBBoxDiff;
function semanticObjects(project) {
...
function computeSemanticBBoxDiff(before, after) {
    const violations = [];
    ...
    for (const obj of semanticObjects(before)) {
        ...
        const sizeChangeRatio = Math.max(Math.abs((next.bbox.w || 1) - (obj.bbox.w || 1)) / Math.max(obj.bbox.w || 1, 1), Math.abs((next.bbox.h || 1) - (obj.bbox.h || 1)) / Math.max(obj.bbox.h || 1, 1));
