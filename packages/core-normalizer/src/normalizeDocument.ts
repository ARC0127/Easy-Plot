...
import { classifyNodeKind } from './classify/basicNodeKinds';
import { parseBBox } from './geometry/bbox';
import { collectInlineStyles } from './style/collectInlineStyles';
import { normalizeTransform } from './transform/normalizeTransform';
import { NormalizedDocument, NormalizedNode } from './types';

interface StyleRule {
  selector: string;
  declarations: Record<string, StyleDeclarationEntry>;
  specificity: number;
  order: number;
}
...
type ExtendedParsedNode = ParsedDocument['nodes'][string] & {
  localTagName?: string;
  namespacePrefix?: string | null;
  namespaceUri?: string | null;
};

type SelectorCombinator = 'descendant' | 'child' | 'adjacent' | 'sibling';
...
const INHERITABLE_STYLE_KEYS: ReadonlySet<string> = new Set([
  'font-family',
  'font-size',
  'font-style',
  'font-weight',
...
function getLocalTagName(node: ParsedDocument['nodes'][string]): string {
  const ext = node as ExtendedParsedNode;
  return normalizeTagName(ext.localTagName ?? node.tagName);
}

function getNamespacePrefix(node: ParsedDocument['nodes'][string]): string | null {
  const ext = node as ExtendedParsedNode;
  return ext.namespacePrefix ?? null;
}

function getNamespaceUri(node: ParsedDocument['nodes'][string]): string | null {
  const ext = node as ExtendedParsedNode;
  return ext.namespaceUri ?? null;
}

function parseStyleDeclarationBlock(raw: string): Record<string, StyleDeclarationEntry> {
  const out: Record<string, StyleDeclarationEntry> = {};
  const normalized = raw.replace(/\/\*[\s\S]*?\*\//g, '');
  for (const [key, value] of Object.entries(collectInlineStyles(normalized.replace(/\n/g, ';')))) {
    if (!key) continue;
...
function splitSelectorList(rawSelectors: string): string[] {
  ...
  for (let i = 0; i < rawSelectors.length; i += 1) {
    ...
    if (quote) {
      if (ch === quote && rawSelectors[i - 1] !== '\\') quote = null;
      chunk += ch;
      continue;
    }
    if (ch === '"' || ch === "'") {
...
function parseStyleRules(parsed: ParsedDocument): StyleRule[] {
  const rules: StyleRule[] = [];
  let order = 0;
  const ruleRe = /([^{}]+)\{([^{}]*)\}/g;

  for (const node of Object.values(parsed.nodes)) {
    if (getLocalTagName(node) !== 'style' || !node.textContent) continue;
...
function tokenizeSelector(selector: string): string[] {
  ...
  while (normalized.length > 0 && [' ', '>', '+', '~'].includes(normalized[0])) normalized.shift();
  while (normalized.length > 0 && [' ', '>', '+', '~'].includes(normalized[normalized.length - 1])) normalized.pop();
...
function parseSelector(selector: string): ParsedSelector | null {
  ...
  if (expectSelector) return null;
  if (combinators.length !== selectors.length - 1) return null;

  return { selectors, combinators };
}

function computeSelectorSpecificity(selector: string): number {
  ...
  for (const token of parts) {
    ...
    const stripped = tokenWithoutPseudo
      ...
      .replace(/\.[A-Za-z0-9_-]+/g, '');
    if (stripped && stripped !== '*') tagCount += 1;
  }
...
function parseToken(token: string): {
  ...
  id: string | null;
  classes: string[];
  attrs: Array<{ key: string; operator: 'exists' | '=' | '~=' | '|=' | '^=' | '$=' | '*='; value: string | null }>;
  pseudo: Array<{ name: string; arg: string | null }>;
} {
  const attrs: Array<{ key: string; operator: 'exists' | '=' | '~=' | '|=' | '^=' | '$=' | '*='; value: string | null }> = [];
...
function matchesNthExpression(rawExpr: string, position: number): boolean {
...
function isSimpleSelectorExpression(selector: string): boolean {
  const parsed = parseSelector(selector);
  return Boolean(parsed && parsed.selectors.length === 1 && parsed.combinators.length === 0);
}

function matchesLang(node: ParsedDocument['nodes'][string], arg: string, allNodes: ParsedDocument['nodes']): boolean {
  const target = arg.trim().toLowerCase();
  if (!target) return false;
  let cursor: ParsedDocument['nodes'][string] | undefined = node;
  while (cursor) {
...
function tokenMatches(node: ParsedDocument['nodes'][string], token: string, allNodes: ParsedDocument['nodes']): boolean {
  ...
  if (parsed.tagName && parsed.tagName !== tag) return false;
  ...
  for (const attr of parsed.attrs) {
    ...
    if (!hit || expected === null) return false;
    const actualValue = actualLower ?? '';
    if (attr.operator === '=' && actualValue !== expected) return false;
    if (attr.operator === '~=' && !actualValue.split(/\s+/).includes(expected)) return false;
    if (attr.operator === '|=' && !(actualValue === expected || actualValue.startsWith(`${expected}-`))) return false;
    if (attr.operator === '^=' && !actualValue.startsWith(expected)) return false;
    if (attr.operator === '$=' && !actualValue.endsWith(expected)) return false;
    if (attr.operator === '*=' && !actualValue.includes(expected)) return false;
  }

  for (const pseudo of parsed.pseudo) {
    if (pseudo.name.startsWith(':')) continue;
    ...
    if (pseudo.name === 'root') {
      if (node.parentNodeId !== null) return false;
      continue;
    }
    ...
    if (pseudo.name === 'last-child') {
      if (selfIndex !== siblings.length - 1) return false;
      continue;
    }
    ...
    if (pseudo.name === 'empty') {
      ...
      if (node.children.length > 0 || text.length > 0) return false;
      continue;
    }
    if (pseudo.name === 'nth-child') {
      const pos = selfIndex + 1;
      const arg = (pseudo.arg ?? '').toLowerCase();
      if (!matchesNthExpression(arg, pos)) return false;
      continue;
    }
    ...
    if (pseudo.name === 'first-of-type' || pseudo.name === 'last-of-type' || pseudo.name === 'only-of-type') {
      ...
      if (pseudo.name === 'first-of-type' && typeIndex !== 0) return false;
      if (pseudo.name === 'last-of-type' && typeIndex !== sameType.length - 1) return false;
      if (pseudo.name === 'only-of-type' && sameType.length !== 1) return false;
      continue;
    }
    ...
    if (pseudo.name === 'lang') {
      if (!matchesLang(node, pseudo.arg ?? '', allNodes)) return false;
      continue;
    }
    if (pseudo.name === 'is' || pseudo.name === 'where') {
      const selectors = splitSelectorList(pseudo.arg ?? '').filter(isSimpleSelectorExpression);
      if (selectors.length === 0) return false;
      if (!selectors.some((selector) => tokenMatches(node, selector, allNodes))) return false;
      continue;
    }
    if (pseudo.name === 'not') {
      const selectors = splitSelectorList(pseudo.arg ?? '').filter(isSimpleSelectorExpression);
...
function selectorMatches(
  node: ParsedDocument['nodes'][string],
  selector: string,
  allNodes: ParsedDocument['nodes']
): boolean {
  const parsed = parseSelector(selector);
  if (!parsed || parsed.selectors.length === 0) return false;

  const getPreviousSibling = (currentNode: ParsedDocument['nodes'][string]): ParsedDocument['nodes'][string] | undefined => {
    if (!currentNode.parentNodeId) return undefined;
    const parent: ParsedDocument['nodes'][string] | undefined = allNodes[currentNode.parentNodeId];
  ...
  let current: ParsedDocument['nodes'][string] | undefined = node;
  const { selectors, combinators } = parsed;

  if (!current || !tokenMatches(current, selectors[selectors.length - 1], allNodes)) return false;
  ...
  for (let i = selectors.length - 2; i >= 0; i -= 1) {
    ...
    if (combinator === 'child') {
      const parent: ParsedDocument['nodes'][string] | undefined = current.parentNodeId ? allNodes[current.parentNodeId] : undefined;
      if (!parent || !tokenMatches(parent, needed, allNodes)) return false;
      current = parent;
      continue;
    }
    ...
    if (combinator === 'adjacent') {
      ...
      if (!sibling || !tokenMatches(sibling, needed, allNodes)) return false;
      current = sibling;
      continue;
    }
    ...
    if (combinator === 'sibling') {
      ...
      let matched: ParsedDocument['nodes'][string] | undefined;
      while (sibling) {
    ...
    let ancestor: ParsedDocument['nodes'][string] | undefined = current.parentNodeId ? allNodes[current.parentNodeId] : undefined;
    let found = false;
    while (ancestor) {
...
function collectPresentationStyles(attributes: Record<string, string>): Record<string, string> {
  const presentationKeys: Array<[string, string]> = [
    ['fill', 'fill'],
  ...
  for (const [attr, key] of presentationKeys) {
    if (attributes[attr] !== undefined) styles[key] = attributes[attr];
  }
  return styles;
}

function applyMatrixToPoint(x: number, y: number, matrix: [number, number, number, number, number, number]): [number, number] {
...
function intersectBBoxes(
  ...
  if (x2 < x1 || y2 < y1) return { x: x1, y: y1, w: 0, h: 0 };
  return { x: x1, y: y1, w: x2 - x1, h: y2 - y1 };
}

function resolveUseReferenceBBox(
  parsedNode: ParsedDocument['nodes'][string],
  parsed: ParsedDocument,
  baseBBox: { x: number; y: number; w: number; h: number } | null
): { x: number; y: number; w: number; h: number } | null {
  if (getLocalTagName(parsedNode) !== 'use') return baseBBox;
  const hrefRaw = parsedNode.attributes.href ?? parsedNode.attributes['xlink:href'] ?? '';
  if (!hrefRaw.startsWith('#')) return baseBBox;
  const targetId = hrefRaw.slice(1);
  const target = Object.values(parsed.nodes).find((node) => String(node.attributes.id ?? '') === targetId);
  if (!target) return baseBBox;
  const targetBBox = parseBBox(target.tagName, getLocalTagName(target), target.attributes);
  if (!targetBBox) return baseBBox;
...
function collectSpecialRegionBBox(
  parsed: ParsedDocument,
  bboxes: Record<string, { x: number; y: number; w: number; h: number } | null>,
  localTag: 'clippath' | 'mask'
): Record<string, { x: number; y: number; w: number; h: number }> {
  const out: Record<string, { x: number; y: number; w: number; h: number }> = {};
  for (const node of Object.values(parsed.nodes)) {
    if (getLocalTagName(node) !== localTag) continue;
...
function getUrlRefId(value: string | undefined): string | null {
  if (!value) return null;
  const match = value.match(/url\(\s*#([^)]+)\s*\)/i);
  return match ? match[1].trim() : null;
}

function parseStyleValue(raw: string): StyleDeclarationEntry {
...
function splitVarArguments(raw: string): { name: string; fallback: string | null } {
  let depth = 0;
  let quote: '"' | "'" | null = null;
...
function resolveCssVarValue(
  ...
  if (!rawValue || depth > 12 || !rawValue.includes('var(')) return rawValue;
...
export function normalizeDocument(parsed: ParsedDocument): NormalizedDocument {
  const parserMetadata = (parsed as ParsedDocument & {
    parseMetadata?: {
  ...
  const resolvedStyleCache: Record<string, Record<string, string>> = {};
  const resolvedCustomPropCache: Record<string, Record<string, string>> = {};

  const resolveCascadeForNode = (nodeId: string): void => {
    if (resolvedStyleCache[nodeId] && resolvedCustomPropCache[nodeId]) return;
    ...
    const inheritedStyles = Object.fromEntries(
      Object.entries(parentStyles).filter(([key]) => INHERITABLE_STYLE_KEYS.has(key))
    ...
    const styleImportance: Record<string, boolean> = {};
    const mergedCustomProps: Record<string, string> = { ...parentCustomProps };
    const customImportance: Record<string, boolean> = {};
    ...
    const applyDeclaration = (key: string, entry: StyleDeclarationEntry) => {
    ...
    const inlineStyles = collectInlineStyles(node.attributes.style);
  ...
  const resolveStylesForNode = (nodeId: string): Record<string, string> => {
    resolveCascadeForNode(nodeId);
    return resolvedStyleCache[nodeId] ?? {};
  };

  const resolveCustomPropsForNode = (nodeId: string): Record<string, string> => {
    resolveCascadeForNode(nodeId);
    return resolvedCustomPropCache[nodeId] ?? {};
  };

  const rawBBoxes: Record<string, { x: number; y: number; w: number; h: number } | null> = {};
  const transforms: Record<string, ReturnType<typeof normalizeTransform>> = {};
  const globalMatrices: Record<string, [number, number, number, number, number, number] | null> = {};
  for (const parsedNode of Object.values(parsed.nodes)) {
    transforms[parsedNode.nodeId] = normalizeTransform(parsedNode.attributes.transform);
  }

  const resolveGlobalMatrix = (nodeId: string): [number, number, number, number, number, number] | null => {
    if (globalMatrices[nodeId] !== undefined) return globalMatrices[nodeId];
    const node = parsed.nodes[nodeId];
    if (!node) return null;
    ...
    let out: [number, number, number, number, number, number] | null = null;
    if (parentMatrix && localMatrix) out = multiplyMatrices(parentMatrix, localMatrix);
    else if (parentMatrix) out = parentMatrix;
    else if (localMatrix) out = localMatrix;
  ...
  for (const parsedNode of Object.values(parsed.nodes)) {
    const baseBBox = parseBBox(parsedNode.tagName, getLocalTagName(parsedNode), parsedNode.attributes);
  ...
  for (const parsedNode of Object.values(parsed.nodes)) {
    ...
    nodes[parsedNode.nodeId] = {
      ...
      nodeKind: classifyNodeKind(parsedNode.tagName, getLocalTagName(parsedNode)),
      bbox,
