...
function attrs(record: Record<string, string | number | null | undefined>): string {
...
function transformAttr(obj: AnyObject): string | undefined {
  const parts: string[] = [];
...
function combineTransformAttrs(...parts: Array<string | undefined | null>): string | undefined {
  const tokens = parts
...
function quoteFontToken(token: string): string {
  return /[\s]/.test(token) && !/^(sans-serif|serif|monospace)$/i.test(token) ? `"${token}"` : token;
}

function splitFontFamilyList(raw: string | null | undefined): string[] {
  return String(raw ?? '')
    .split(',')
...
function buildExportFontFamily(raw: string | null | undefined): string | undefined {
  ...
  for (const token of [...explicit, ...fallbackStack]) {
    ...
    if (!cleaned) continue;
    if (merged.some((entry) => entry.toLowerCase() === cleaned.toLowerCase())) continue;
    merged.push(cleaned);
  }
  return merged.length > 0 ? merged.map(quoteFontToken).join(', ') : undefined;
}

function textTransformAttr(obj: AnyObject & { position: [number, number] }): string | undefined {
  const rotate = obj.transform.rotate;
...
function renderGlyphProxyText(
  ...
  const wrapperStyle = (glyphVector.wrapperStyle ?? {}) as Record<string, string>;
...
function resolveShapeStyleAttrs(obj: Extract<AnyObject, { objectType: 'shape_node' }>, shapeAttrs: Record<string, string>): Record<string, string | number> {
  ...
  const fallbackStrokeColor =
    obj.shapeKind === 'line' || obj.shapeKind === 'polyline'
      ? '#0f172a'
      : '#334155';
  const fallbackStrokeWidth = obj.shapeKind === 'line' || obj.shapeKind === 'polyline' ? 1.5 : 1;
  const resolvedStroke: string | undefined = hasExplicitStroke
    ? obj.stroke.color
    : hasAttrStroke
      ? undefined
      : hasFill
  ...
  const resolvedStrokeWidth = hasExplicitStroke
    ...
    : hasAttrStroke
      ...
      : hasFill
        ? undefined
        : fallbackStrokeWidth;
  const resolvedDasharray: string | undefined = hasExplicitStroke ? obj.stroke.dasharray ?? undefined : undefined;
  const resolvedFill: string | undefined = hasExplicitFill
    ? obj.fill.color ?? undefined
    : hasAttrFill
      ? undefined
      : 'none';
  const resolvedFillOpacity =
    hasExplicitFill && obj.fill.color !== null && obj.fill.opacity !== 1 ? obj.fill.opacity : undefined;
  const styleAttrs: Record<string, string | number> = {};
  if (resolvedStroke !== undefined && resolvedStroke !== null) styleAttrs.stroke = resolvedStroke;
  if (resolvedStrokeWidth !== undefined) styleAttrs['stroke-width'] = resolvedStrokeWidth;
  if (resolvedDasharray !== undefined && resolvedDasharray !== null) styleAttrs['stroke-dasharray'] = resolvedDasharray;
  if (resolvedFill !== undefined && resolvedFill !== null) styleAttrs.fill = resolvedFill;
  if (resolvedFillOpacity !== undefined) styleAttrs['fill-opacity'] = resolvedFillOpacity;
  if (hasStroke || !hasFill) styleAttrs['vector-effect'] = 'non-scaling-stroke';
  return styleAttrs;
}
...
function collectUseDefinitions(renderables: AnyObject[]): string[] {
  const defsById = new Map<string, string>();
  for (const obj of renderables) {
    if (obj.objectType !== 'shape_node') continue;
    const useReference = (obj.geometry as any)?.useReference;
    ...
    const definitionAttributes = (useReference.definitionAttributes ?? {}) as Record<string, string>;
    if (!definitionId || !definitionTag) continue;
    if (defsById.has(definitionId)) continue;
    defsById.set(
      definitionId,
      `<${definitionTag} ${attrs({ ...definitionAttributes, id: definitionId })} />`
    );
  }
...
function renderObject(obj: AnyObject, parentId: string | null = null): string {
  ...
  switch (obj.objectType) {
    ...
    case 'shape_node': {
      const shapeAttrs = (obj.geometry as any)?.attributes ?? {};
      const originalTag = String((obj.geometry as any)?.originalTag ?? '').toLowerCase();
      const useReference = (obj.geometry as any)?.useReference ?? null;
      ...
      return `<${shapeKind} ${attrs({
        ...
        ...shapeAttrs,
        ...geometryFallback,
        transform: combineTransformAttrs(
          typeof shapeAttrs.transform === 'string' ? shapeAttrs.transform : undefined,
          transformAttr(obj)
        ),
        ...styleAttrs,
      })} />`;
